...
class AusenteDataTable extends DataTable
{
    /**
    ...
    {
        return $model->newQuery()->where('tipo', 1)->where('presente', false);
    }
    ...
    {
        return $this->builder()
            ->setTableId('ausente-sinodal-datatable')
            ->columns($this->getColumns())
            ->minifiedAjax(route('admin.datatable.ausente-sinodal'))
            ->dom('Bfrtip')
            ->orderBy(1)
            ->buttons(['csv'])
            ->parameters([
